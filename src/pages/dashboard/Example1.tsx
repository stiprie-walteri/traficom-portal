      ...
      {showCommentsList && (
        ...
        >
          <div className="flex justify-between items-start mb-3">
            <h3 className="font-semibold text-sm text-gray-900">All Comments ({textsComments.length})</h3>
            <button
          ...
          <div className="space-y-2">
            {textsComments.slice(0, 3).map((item) => (
              ...
              >
                <p className="text-xs text-gray-800 font-medium mb-1 line-clamp-2">{item.text}</p>
                <p className="text-xs text-gray-600 line-clamp-1">{item.comment}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {activeComment && (
        ...
        >
          ...
          {activeComment.references && activeComment.references.length > 0 && (
            <div className="mb-3">
              <h4 className="font-semibold text-xs text-gray-900 mb-1">References</h4>
              <ul className="text-xs text-gray-700 space-y-1">
                {activeComment.references.map((ref, index) => (
                  <li key={index} className="pl-2 border-l-2 border-gray-500">{ref}</li>
                ))}
              </ul>
            </div>
          )}
          <div className="flex items-center justify-between pt-3 border-t border-gray-300">
            ...
            <span className="text-xs text-gray-600">
              {textsComments.findIndex(tc => tc.id === activeComment.id) + 1} / {textsComments.length}
            </span>
            <button
